...
장소에 대해서 회원이 작성한 후기를 저장하는 엔티티인 PlaceReview가 있습니다.  
...
```kotlin
interface PlaceReviewConfigureMappingRepository : JpaRepository<PlaceReviewConfigureMapping, String> {

    @EntityGraph(attributePaths = ["placeReviewConfigure"])
    fun findAllByPlaceReview(placeReview: PlaceReview): List<PlaceReviewConfigureMapping>
...
EntityGraph를 통해 PlaceReviewConfigureMapping을 가져올 때 한 번에 각 후기당 회원이 선택한 태그정보들을 모두 가져옴으로써 M문제를 해결하였습니다.  
...
우선 PlaceReviewTagMappingRepository에서 **findAllByPlaceReviewIn** 메소드를 새로 정의하고 "placeReviewConfigure", "placeReview"까지 EntityGraph를 설정합니다.  
...
fun getPlaceReviews(
        ...
        var reviewList = allReviewsConfigListMap.map {
            val tagList = it.value.sortedBy { placeReviewConfigureMapping ->
+                        placeReviewConfigureMapping.placeReviewConfigure.seq }
...
allReviewsConfigListMap에서 map을 사용해 placeReviewConfigureMapping.placeReviewConfigure.seq을 기준으로 정렬한 tagList를 구해 ReviewInfo를 만들어 주고, 최종적으로 reviewList를 생성합니다.  
