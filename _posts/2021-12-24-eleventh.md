...
그 이유는 나중에 알아보니 Integer의 경우 값을 변경하면 new를 이용한 것처럼 새로운 객체를 할당하게 되기 때문입니다.<br> find 메소드의 매개변수 Integer(count = 0)는 그냥 참조변수입니다.<br>
**Integer count가 find매개변수 입력되는 순간 이 아이는 더이상 main에 있는 그 아이가 아닙니다.**<br>
...
그래서 find메소드에서 Integer의 값을 변경하는 순간(ex. count =2)에 새로운 Integer객체(2)가 생성되고, 이 주소값을 find함수의 지역참조변수 값에 저장합니다.<br>
...
그래서 그 지역참조변수에서 new를 하는 순간에 이 지역참조변수는 다른 객체를 가리키고, 거기서 값을 변경했을 때, 이 참조변수를 반환값으로 누가 받아주지 않으면 메소드 종료와 함께 소멸됩니다.<br>
그래서 결국 자바에서는 c++처럼 포인터를 사용할 수 없기 때문에 메소드에서 출력값을 여러개로 설정할 수 없고, 하나의 출력값만을 설정할 수 있습니다.
### 유의사항
여기서 헷갈리면 안되는 것이 main에서 Persoanl객체(String name을 멤버로 가지고 있는)를 가리키는 참조변수의 값을 void chanegeName(Personal personal)의 매개변수로 입력하여 그 안에서 name을 변경하면 메소드 종료 후에 이름이 바뀐다는 점입니다!!!<br>
